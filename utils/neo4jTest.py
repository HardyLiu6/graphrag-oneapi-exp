...
该脚本用于将 GraphRAG 生成的 Parquet 文件数据导入到 Neo4j 图数据库中。
包括文档、文本块、实体、关系、社区、协变量等多种节点和关系的创建。

数据库连接信息:
    Neo4j URI: neo4j+s://badbd4fd.databases.neo4j.io
    Database: neo4j
...
# 3、NEO4J 数据库查询使用方法
# （1）显示几个 __Entity__ 节点及其关系（实体图）
# MATCH path = (:__Entity__)-[:RELATED]->(:__Entity__)
# RETURN path LIMIT 200
# （2）显示 Chunks 和 Document（词汇图）
...
# （2）关系包括:
# RELATED(entity 之间)
# PART_OF(chunk 与 document 之间)
# HAS_ENTITY(chunk 与 entity 之间)
# IN_COMMUNITY(entity 与 community 之间)
# HAS_FINDING
# HAS_COVARIATE(chunk 与 covariate 之间)
...
# 指定 Parquet 文件路径
GRAPHRAG_FOLDER="D:/PythonWork/RAG/graph_test/ragtest/inputs/artifacts"


# 数据库连接相关参数配置
NEO4J_URI="neo4j+s://badbd4fd.databases.neo4j.io"
NEO4J_USERNAME="neo4j"
NEO4J_PASSWORD="QZr56KnlkarTWNjzEoDKmNwOxO7Im01gvhc_H-uakyY"
...
def batched_import(statement, df, batch_size=1000):
    ...
    参数:
        ...
        df: 要导入的数据帧
        batch_size: 每批要导入的行数，默认为 1000
    
    返回:
        导入的总行数
    """
    # 计算数据帧 df 中的总行数，并将其存储在 total 变量中
    total = len(df)
    # 记录当前时间，以便后续计算导入操作所花费的总时间
    start_s = time.time()
    # 每次循环处理一批数据，步数为 batch_size
    for start in range(0, total, batch_size):
        # 使用 Pandas 的 iloc 方法提取当前批次的数据子集
        # start 是当前批次的起始行号
        # min(start + batch_size, total) 是当前批次的结束行号，确保不会超过总行数
        batch = df.iloc[start: min(start+batch_size, total)]
        # "UNWIND $rows AS value " 是 Cypher 中的一个操作，它将 $rows 中的每个元素逐个解包，并作为 value 传递给 Cypher 语句
    ...
    # 计算并打印导入总行数和耗时
    print(f'{total} 行在 {time.time() - start_s} 秒内导入完成。')
    # 返回导入的总行数
    return total
...
# WITH c, value 用于将当前查询上下文中的变量传递给接下来的查询部分。在这里，c 和 value 被传递到下一步的查询中
# UNWIND value.document_ids AS document 将列表 value.document_ids 中的每个元素依次展开为单独的记录，并将每个元素命名为 document，进行单独处理
# MATCH (d:__Document__ {id:document}) 查找 __Document__ 标签的节点，并且 id 属性值等于 document
# MERGE (c)-[:PART_OF]->(d) 在 __Chunk__ 节点与 __Document__ 节点之间创建一个 PART_OF 类型的关系。如果关系已经存在，则不创建新的关系，表示 c 是 d 的一部分
statement = """
